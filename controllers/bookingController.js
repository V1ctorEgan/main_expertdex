...
const { BOOKING_STATUS, TRIP_DURATION_MINUTES } = require('../config/bookingStatus');
...
const createNewBooking = async (req, res, next, io) => {
    ...
    try {
        ...
        // Find and update the vehicle's availability in a single operation.
        // The update will only succeed if the vehicle is currently available.
        ...
        if (err instanceof Types.ObjectId) {
...
const assignDriverToBooking = async (req, res, next, io) => {
    ...
    try {
        ...
        const endTime = new Date(startTime.getTime() + TRIP_DURATION_MINUTES * 60000); // Add estimated trip duration
        const conflictingBooking = await Booking.findOne({
            ...
            $or: [
                {
                    // Check for overlap where the new booking starts during an existing booking
                    scheduledTime: { $gte: startTime, $lt: endTime }
                },
                {
                    // Check for overlap where an existing booking starts during the new booking
                    scheduledTime: { $lt: startTime },
                    completionTime: { $gte: startTime }
                }
            ]
        }).exec();
        ...
        booking.status = BOOKING_STATUS.ACCEPTED; // Automatically mark the booking as accepted once a driver is assigned

        const result = await booking.save();

        // --- Emit a real-time event to notify the customer and driver ---
        const customerId = booking.customerId.toString();
        const driverUserId = driverProfile.userId.toString(); // assuming driverProfile has a userId field
        ...
        // Notify the driver they have been assigned a new booking
        io.to(driverUserId).emit('new-booking-assignment', {
